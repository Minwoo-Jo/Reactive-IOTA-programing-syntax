...
public class Cell<A> {
	...
	/**
	 * @return The value including any updates that have happened in this
	 *         transaction.
	 */
	final A newValue() {
	 ...
	 * <p>
	 * It may be used inside the functions passed to primitives that apply them to
	 * {@link Stream}s, including {@link Stream#map(Lambda1)} in which case it is
	 * equivalent to snapshotting the cell, {@link Stream#snapshot(Cell, Lambda2)},
	 * {@link Stream#filter(Lambda1)} and {@link Stream#merge(Stream, Lambda2)}. It
	 * should generally be avoided in favour of {@link listen(Handler)} so you don't
	...
	/**
	 * A variant of {@link sample()} that works with {@link CellLoop}s when they
	 * haven't been looped yet. It should be used in any code that's general enough
	 * that it could be passed a {@link CellLoop}.
	 * 
	...
	/**
	 * Transform the cell's value according to the supplied function, so the
	 * returned Cell always reflects the value of the function applied to the input
	 * Cell's value.
	 * 
	 * @param f
	 *            Function to apply to convert the values. It must be
	...
	/**
	 * Lift a binary function into cells, so the returned Cell always reflects the
	...
	/**
	 * Lift a ternary function into cells, so the returned Cell always reflects the
	 * specified function applied to the input cells' values.
	 * 
	 * @param fn
	 *            Function to apply. It must be <em>referentially transparent</em>.
	...
	/**
	 * Lift a quaternary function into cells, so the returned Cell always reflects
	 * the specified function applied to the input cells' values.
	 * 
	 * @param fn
	 *            Function to apply. It must be <em>referentially transparent</em>.
	 */
	...
	/**
	 * Lift a 5-argument function into cells, so the returned Cell always reflects
	 * the specified function applied to the input cells' values.
	 * 
	 * @param fn
	 *            Function to apply. It must be <em>referentially transparent</em>.
	 */
	public final <B, C, D, E, F> Cell<F> lift(Cell<B> b, Cell<C> c, Cell<D> d, Cell<E> e,
			final Lambda5<A, B, C, D, E, F> fn) {
	...
	/**
	 * Lift a 6-argument function into cells, so the returned Cell always reflects
	 * the specified function applied to the input cells' values.
	 * 
	 * @param fn
	 *            Function to apply. It must be <em>referentially transparent</em>.
	 */
	public final <B, C, D, E, F, G> Cell<G> lift(Cell<B> b, Cell<C> c, Cell<D> d, Cell<E> e, Cell<F> f,
			final Lambda6<A, B, C, D, E, F, G> fn) {
	...
	public static <A, B> Cell<B> apply(final Cell<Lambda1<A, B>> bf, final Cell<A> ba) {
		return Transaction.apply(new Lambda1<Transaction, Cell<B>>() {
			public Cell<B> apply(Transaction trans0) {
				...
				return out.lastFiringOnly(trans0).unsafeAddCleanup(l1).unsafeAddCleanup(l2)
	 ...
	 * The returned {@link Listener} has a {@link Listener#unlisten()} method to
	 * cause the listener to be removed. This is an OPERATIONAL mechanism is for
	 * interfacing between the world of I/O and for FRP.
	 * 
	 * @param action
	 *            The handler to execute when there's a new value. You should make
	 *            no assumptions about what thread you are called on, and the
	 *            handler should not block. You are not allowed to use
	 *            {@link CellSink#send(Object)} or {@link StreamSink#send(Object)}
	 *            in the handler. An exception will be thrown, because you are not
	...
	/**
	 * A variant of {@link listen(Handler)} that will deregister the listener
	 * automatically if the listener is garbage collected. With
	 * {@link listen(Handler)}, the listener is only deregistered if
	 * {@link Listener#unlisten()} is called explicitly.
	 */
	 ...
	 *             {@link lift(Cell,Cell,Cell,Lambda4)}
	 */
	@Deprecated
	public static final <A, B, C, D, E> Cell<E> lift(final Lambda4<A, B, C, D, E> fn, Cell<A> a, Cell<B> b, Cell<C> c,
			Cell<D> d) {
	 ...
	 *            Function to apply. It must be <em>referentially transparent</em>.
	 * @deprecated As of release 1.1.0, replaced by
	 *             {@link lift(Cell,Cell,Cell,Cell,Lambda5)}
	 */
	@Deprecated
	public static final <A, B, C, D, E, F> Cell<F> lift(final Lambda5<A, B, C, D, E, F> fn, Cell<A> a, Cell<B> b,
			Cell<C> c, Cell<D> d, Cell<E> e) {
	 ...
	 *            Function to apply. It must be <em>referentially transparent</em>.
	 * @deprecated As of release 1.1.0, replaced by
	 *             {@link lift(Cell,Cell,Cell,Cell,Cell,Lambda6)}
	 */
	@Deprecated
	public static final <A, B, C, D, E, F, G> Cell<G> lift(final Lambda6<A, B, C, D, E, F, G> fn, Cell<A> a, Cell<B> b,
			Cell<C> c, Cell<D> d, Cell<E> e, Cell<F> f) {
